...
function drawHeader(doc: PDFKit.PDFDocument, pageTitle: string, policyNumber: string) {
...
function drawFooter(doc: PDFKit.PDFDocument, pageNum: number, totalPages: number = 20) {
  const y = 755;
...
export function generatePolicyPDF(policy: any): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    doc.fillColor('#94a3b8').fontSize(7.5).font('Helvetica').text(
      `Toll-Free Helpline: 1800-200-INSURE\n` +
      `WhatsApp Support: +91 98765 00000\n` +
      `TPA Hospitalization Desk: Active\n` +
      `Network Hospitals: 10,500+ Nationwide`,
      332, 648, { width: 215, lineGap: 3 }
    );
    ...
    doc.fillColor('#64748b').fontSize(8).font('Helvetica').text(
      '© 2026 InsureCore General Insurance Corporation. All rights reserved.\n' +
      'Registered Office: InsureCore Tower, BKC, Bandra East, Mumbai - 400051, India.\n' +
      'IRDAI Reg No. 154 | CIN: L66010MH2002PLC136741',
      50, 680, { align: 'center', lineGap: 4 }
    );
...
// 3. TAX CERTIFICATE
export function generateTaxCertificatePDF(policy: any): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    const doc = new PDFDocument({ margin: 40, size: 'A4' });
    const buffers: Buffer[] = [];
...
// 4. SMART HEALTH CARD
export function generateHealthCardPDF(policy: any): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    const doc = new PDFDocument({ margin: 40, size: 'A4' });
    const buffers: Buffer[] = [];
